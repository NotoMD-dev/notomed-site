...
export function RegimenSummary() { // Removed showPrnArea and setShowPrnArea from props
  ...
  return (
    <section className="mt-6"> {/* Removed shadow/border/bg classes */}
      ...
      {isMounted && (
        <div className="bg-[color:var(--hero-tone-pear-surface)] border border-[color:var(--hero-tone-pear-border)] rounded-xl p-4">
          <pre
            className="w-full p-3 bg-white border border-[#e1d7c8] rounded-lg text-sm whitespace-pre-wrap font-mono text-[color:var(--hero-tone-pear-title)]"
          >
            {planText}
          </pre>
        </div>
