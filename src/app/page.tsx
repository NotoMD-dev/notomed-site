                ...
                <Link key={tool.id} href={tool.path} className="block focus-visible:outline-none">
                  <GlowCard
                    className={cn(
                      "min-h-[230px]",
                      isFeatured &&
                        "featured-card-glow border border-[var(--accent)]/60 shadow-[0_12px_42px_rgba(210,126,88,0.22)] ring-1 ring-[var(--accent)]/25 transition-transform duration-500 hover:-translate-y-2 hover:scale-[1.03]",
                    )}
...
function getHeroTagClasses(tag: ToolTag) {
  ...
  switch (tag) {
    case "FEATURED":
      return `${base} border border-[var(--hero-tone-sage-border)] bg-[var(--hero-tone-sage-surface)] text-[var(--hero-tone-sage-title)] shadow-[0_12px_30px_rgba(0,0,0,0.18)]`;
    case "NEW":
